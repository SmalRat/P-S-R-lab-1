...
naiveBayes <- setRefClass("naiveBayes",
       ...
       fields = list(data = "data.frame", labels = "vector", labels_quantity = "numeric"),
       methods = list(
                    ...
                    predict = function(message)
                    {
                         ...
                         words <- strsplit(removed, " +")[[1]]
                         label_probability_vector = vector(mode="double", length=length(words)*length(labels))
                         relevant_data = data[data$Word %in% words, ]
                         print(relevant_data)
                         for (word in words){
                           if (!(word %in% data$Word)){
                             relevant_data[nrow(relevant_data)+1,] <- c(word, 0, 123, 0, 123)
                             print(word)
                           }
                         }
                         print(relevant_data)
                         ...
                         for (label in seq(1,length(labels))){
                            for (word in seq(1,length(words))){
                              ...
                              probability <- data[data$Word==words[word],][column_name]
                              label_probability_vector[word + (label-1)*length(words)] = probability
                              
                            }
                         }
                         print(label_probability_vector[[1]])
...
model = naiveBayes(labels=unique(tidy_text$label), data=data.frame("Word" = unlist(words_list)), labels_quantity=length(unique(tidy_text$label)))
model$fit(X=tidy_text)
model$predict("Trump black wertafyyfay")
```
