...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    predict = function(message)
                    {
                         ...
                         idxs <- c()
                         range <- 5:length(multiplication_vector)
                         for (i in range) {
                    ...
                    score = function(filepath)
                    {
                      ...
                      test <-  read.csv(file = filepath, stringsAsFactors = FALSE)
                      predictions = vector(length=nrow(test))
                      
                      
                      #remove first line in df
                      count = 1
                      ...
                      for (message in test$tweet) {
                        ...
                        if (predictions[count] == 1) {
                          ...
                          predicitons <- append(predicitons, "discrim")
                        } else {
                          predictions <- append(predictions, "neutral")
                        }
                        count = count + 1
                      }
                      print(predictions)
                      ...
                      data1 <- data1[, -1]
                      data1 <- data1[, -1]
                      names(data1) <- "test set"
                      data1$prediction <- predictions
                      counter = 0
                      for (row in 1:nrow(data1)) {
                        verified <- data1[row, "test set"]
                        predict  <- data1[row, prediction]
                        if (verified == predict) {
                          counter = counter + 1
                        }
                      }
...
model$fit(X=tidy_t)
model$predict("Trump black wertafyyfay the in a zzzzzzzz")
