...
neutral_posts_text <- filter(tidy_text, label == "neutral")
neutral_posts_counted <- count(neutral_posts_text, Word, sort=TRUE)
neutral_posts_text
neutral_posts_counted

discrimination_posts_text <- filter(tidy_text, label == "discrim")
discrimination_posts_counted <- count(discrimination_posts_text, Word, sort=TRUE)
discrimination_posts_text
discrimination_posts_counted
...
naiveBayes <- setRefClass("naiveBayes",
       ...
       fields = list(data = "data.frame", labels = "vector"),
       methods = list(
                    ...
                    fit = function(X, y)
                    {
                         ...
                         for (i in seq(1,length(unlist(labels)))){
                           ...
                           current_topic[column_name] = ((current_topic["n"]+1)/(sum(current_topic$n) + length(unique(current_topic$Word))))
                          
                           data <<- merge(data, current_topic, by = "Word", all = TRUE)
                           
                         }
                         data <<- data[order(data$n.x, decreasing = TRUE), ]
                         data[is.na(data)] <<- 0
                         print(data)
                    ...
                    predict = function(message)
                    {
                         prob_neutral = 1
                         prob_discrim = 1
                         removed <- tolower(message) %>% removePunctuation() %>% removeNumbers() %>% stripWhitespace()
                         removed
                         words <- strsplit(removed, " +")
                         words <- words[[1]]
                         print(words)
                         print(labels)
                         ...
                         for (label in seq(1,length(labels))){
                            label_probability_vector = vector(mode="double", length=length(words))
                            for (word in seq(1,length(words))){
                              print(labels[label])
                              print(words[word])
                             # row <- which(rownames(discrimination_posts_counted) == i)
                             # val <- discrimination_posts_counted[row, 3]
                              column_name = paste("P(word|", labels[label], ")")
                              probability <- data[data$Word==words[word],][column_name] #[match(i, discrimination_posts_counted$discrimination_posts_counted[,1])]
                              label_probability_vector[word] = probability
                              
                            }
                            
                            labels_probabilities <- append(labels_probabilities, label_probability_vector)
...
words_list = list(unique(tidy_text$Word))
model = naiveBayes(labels=unique(tidy_text$label), data=data.frame("Word" = unlist(words_list)))
model$fit(X=tidy_text)
model$predict("Trump black")
```
